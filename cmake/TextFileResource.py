...
class StyleInfo:
    def __init__(self, file_matches = (), comment_regex = None, include_regex = None, priority = 0):
...
def buildStyles():
    ...
    c_style_comment = re.compile(r'(^[ \t\r]*\n)|(^[ \t]*//.*\n)|(//.*$)|(/\*.*\*/)', re.M)
    # C/C++ include regular expression
    c_style_include = re.compile(r'^[ \t]*#include[ \t]+[<"](.*)[>"].*$', re.M)
    styles = {
        'none': StyleInfo(),
        'text': StyleInfo(),
        'c': StyleInfo(['c', 'h'], c_style_comment, c_style_include),
        'c#': StyleInfo(['cs'], c_style_comment),
        'cmake': StyleInfo(['cmake', 'CMakeLists.txt'], re.compile(r'#.*$', re.M)),
        'cpp': StyleInfo(['cpp', 'cxx', 'h', 'hpp', 'hxx'], c_style_comment, c_style_include, priority = 1),
        'cuda': StyleInfo(['cu'], c_style_comment, c_style_include),
        'java': StyleInfo(['java'], c_style_comment),
        'opencl': StyleInfo(['cl'], c_style_comment, c_style_include),
        'python': StyleInfo(['py'], c_style_comment)
    }
    return styles
...
parser.add_argument('--src-ext', type = str, default = 'cpp',
                   help = 'File extention for the source file. Default is cpp.')
parser.add_argument('--h-ext', type = str, default = 'h',
                   help = 'File extention for the header file.')
parser.add_argument('--mode', type = str, default = 'text', choices = ('binary', 'text'),
                    help = 'Output mode control; text mode writes as readable text, while bytes write as an array of ' +
                            'bytes. Bytes are also written if the string content is too long.')
parser.add_argument('--style', type = str, default = 'none', choices = text_file_styles.keys(),
                    help = 'Explicitly specify the file type. Affects comment and include directive handling. ' +
                            'Matched by extension when set to none. Use "text" for no style.')
parser.add_argument('--comment-regex', type = str,
                    help = 'Override for the regular expression used to match comments for minification.' +
                            'Applied as a multi-line regular expression.')
parser.add_argument("--name", type = str,
                    help = "Specifies the output variable name. The default is '<filename>Resource' without the path.")
parser.add_argument('--minify', type = str, default = 'off', choices = ('off', 'on', 'comments', 'whitespace'),
...
parser.add_argument('--no-import', type = str2bool, const = True, nargs='?',
                    help = 'Do not expand include/import statements?')
parser.add_argument('--include-regex', type = str,
                    help = 'Override for the regular expression used to "include" other files. Expression is ' +
                            'replaced by the file identified in capture 1. This file is searched on the -I paths')
parser.add_argument('-I', type = str, dest = 'include_dirs', action='append',
                    help = 'Specifies a path used to resolve include statements. May be specified multiple times.')
...
def resolveIncludeDirectives(regex, content, include_dirs):
    ...
    while directive:
        ...
        if include_file_path:
            ...
            print("Failed to resolve include", directive.group(0), ":", directive.group(1))
...
if style.include_regex and not args.no_import:
    input_file_content = resolveIncludeDirectives(style.include_regex, input_file_content, args.include_dirs)
...
if input_file_content_length < 1 << 16 and args.mode == 'text':
    ...
    input_file_content_string = re.sub(r'\\(?!")', r'\\\\', input_file_content_string)
