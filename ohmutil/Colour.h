...
struct ohmutil_API Colour
{
  ...
  enum { kR = 0, kG = 1, kB = 2, kA = 3 };
  /// Colour shift values used in conversion to/from 32-bit integer.
  enum { kAShift = 24, kRShift = 16, kGShift = 8, kBShift = 0 };
  ...
  /// @param c The colour to copy.
  /// @return This.
  inline Colour &operator = (const Colour &c) { rgb[kR] = c.rgb[kR]; rgb[kG] = c.rgb[kG]; rgb[kB] = c.rgb[kB]; rgba[kA] = c.rgba[kA]; return *this; }
