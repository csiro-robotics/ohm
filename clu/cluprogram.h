...
//
#ifndef CLUPROGRAM_H_
#define CLUPROGRAM_H_
...
namespace clu
{
  ...
  void printBuildLog(std::ostream &out, const cl::Program &program, const cl::Device &device, int filterStatus = 1);
  ...
  /// Logging is filtered based on the build status when @p filterStatus is a valid
  /// @c cl_build_status value. Otherwise any status passes.
  void printBuildLogs(std::ostream &out, const cl::Program &program, const cl::Context &context, int filterStatus = 1);
  ...
  /// @param localMemFunc Optional function to calculate Local memory required based on
  ///     work groups size.
  ...
  /// @param bufferLength The capacity of @p cwd in characters.
  /// @return @c cwd
  char *currentWorkingDirectory(char *cwd, size_t bufferLength);
  ...
  /// @param bufferLength The buffer capacity of @p fileName (in characters).
  /// @param searchPaths Additional search paths in a comma separated list.
  /// @return A pointer to @c fileName on success, null on failure.
  char *findProgramDir(char *fileName, size_t bufferLength, const char *searchPaths = nullptr);
  ...
  char *findProgramPath(char *fileName, size_t bufferLength, const char *searchPaths = nullptr);
  ...
  /// @param debugOption Debug option to be added and postfixed with the full source file name.
  /// @param sourceFileOpt Compiler option used to specify the path to the source file.
  ...
  /// @param searchPaths Additional search paths for the @p sourceFileName. Use comma as a path separator.
  cl_int buildProgramFromFile(cl::Program &program, cl::Context &ocl,
                              std::string &sourceFileName,
                              std::ostream &log,
                              const std::vector<std::string> *args,
                              const char *debugOption = nullptr,
