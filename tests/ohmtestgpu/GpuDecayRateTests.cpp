...
TEST(DecayRate, Through)
{
  ...
  ohmtestcommon::decay::testThrough(map, mapper, [&mapper] { mapper.syncVoxels(); });
}

TEST(DecayRate, ThroughNoMean)
{
  // Same as Through, but without sub-voxel positioning. Realistically this isn't useful information as the decay rate
  ...
  ohm::OccupancyMap map(0.1f, ohm::MapFlag::kDecayRate);
  ohm::GpuMap mapper(&map, true);
  ohmtestcommon::decay::testThrough(map, mapper, [&mapper] { mapper.syncVoxels(); });
}
...
TEST(DecayRate, Into)
{
  ...
  ohm::OccupancyMap map(0.1f, ohm::MapFlag::kVoxelMean | ohm::MapFlag::kDecayRate);
  ohm::GpuMap mapper(&map, true);
  ohmtestcommon::decay::testInto(map, mapper, [&mapper] { mapper.syncVoxels(); });
}

TEST(DecayRate, IntoNoMean)
{
