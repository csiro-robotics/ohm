...
namespace clu
{
  ...
  {
    return ::clSetKernelArg(kernel(), arg_index, sizeof(cl_mem), arg.arg<cl_mem>());
  }
  ...
  {
    return ::clSetKernelArg(kernel(), arg_index, sizeof(cl_mem), arg.buffer.arg<cl_mem>());
  }
}  // namespace clu
...
namespace gputil
{
                         ...
                         ARGS... args)
  {
    ...
    if (events_clu.event_count)
    {
      ...
      {
        events_clu.wait_on_events[i] = event_list.events()[i].detail()->event;
      }
    }
    ...
    grid.global_size = clu::KernelSize(global_size.x, global_size.y, global_size.z);
    cl::CommandQueue &queue_cl = (queue) ? queue->internal()->queue : device().defaultQueue().internal()->queue;
    return detail()->kernel(queue_cl, grid, events_clu, args...);
  }
