...
#ifndef OCCUPANCYQUERY_H_
#define OCCUPANCYQUERY_H_
...
namespace ohm
{
  ...
  {
  protected:
    /// Constructor. The @p detail is stored in @p _imp, allowing derived classes to a allocate
  ...
  public:
    ...
    ///
    /// @param hardReset True for a hard reset, false for a soft reset.
    void reset(bool hardReset = true);

    /// Wait for an asynchronous query to complete.
    ///
    /// @param timeoutMs Maximum amount of time to wait for completion (milliseconds).
    ...
    bool wait(unsigned timeoutMs = ~0u);
