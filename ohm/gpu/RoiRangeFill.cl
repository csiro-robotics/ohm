...
///   - @c w set to 1 for an occupied voxel and zero for a free voxel. Uncertain voxels attain either depending @c unknownAsOccupied
...
bool onROIEdge(int3 coord, int3 roiExtents)
{
  return
    // in region
    0 <= coord.x && coord.x < roiExtents.x &&
    0 <= coord.y && coord.y < roiExtents.y &&
    0 <= coord.z && coord.z < roiExtents.z &&
    // on edge
                                    ...
                                    float3 axisScaling)
{
  ...
  // reason.
  const int linearIndexN = (get_local_id(0) + 1 + nx) +
                           (get_local_id(1) + 1 + ny) * (get_local_size(0) + 2) +
  ...
  // out of range.
  const bool replaceCurrent = neighbourObstacle.w && (currentClosest.w == 0 || neighbourObstacleRangeSqr < *currentDistSqr);
                          ...
                          local int3 *faceVolumes)
{
  ...
  if (get_local_id(0) < 6)
  {
    ...
    case FaceBottom:
      ...
      maxFaceExtents[FaceBottom] = make_int3(workingVoxelExtents.x + outerRegionPadding.x,
                                        workingVoxelExtents.y + outerRegionPadding.y,
                                        0);
      break;
    case FaceTop:
      minFaceExtents[FaceTop] = make_int3(-outerRegionPadding.x,
                                      -outerRegionPadding.y,
                                      workingVoxelExtents.z);
      maxFaceExtents[FaceTop] = workingVoxelExtents + outerRegionPadding;
      break;
    case FaceBack:
      minFaceExtents[FaceBack] = make_int3(-outerRegionPadding.x,
                                      -outerRegionPadding.y,
                                      0);
      maxFaceExtents[FaceBack] = make_int3(workingVoxelExtents.x + outerRegionPadding.x,
                                      0,
                                      workingVoxelExtents.z);
      break;
    case FaceFront:
      minFaceExtents[FaceFront] = make_int3(-outerRegionPadding.x,
                                        workingVoxelExtents.y,
                                        0);
    ...
    case FaceRight:
      ...
      maxFaceExtents[FaceRight] = make_int3(workingVoxelExtents.x + outerRegionPadding.x,
                                      workingVoxelExtents.y,
                                      workingVoxelExtents.z);
      break;
    default:
                                ...
                                int3 globalWorkItem, int3 localWorkItem, int3 localExpanse)
{
  ...
  for (uint i = 0; i < totalLoad; i += volumeOf(localGroupExpanse))
  {
    ...
    uint localLinearIndex = i +
                            localWorkItem.x +
                            localWorkItem.y * localGroupExpanse.x +
    ...
    int3 globalSrcCoord = make_int3(localSrcCoord.x + get_group_id(0) * localExpanse.x,
                                    localSrcCoord.y + get_group_id(1) * localExpanse.y,
  ...
  if (isGlobalThread(debugThread.x, debugThread.y, debugThread.z))
  {
    ...
    for (uint i = 0; i < volumeOf(localMemExpanse); ++i)
    {
      if (localVoxels[i].x == localMemClearValue.x &&
          localVoxels[i].y == localMemClearValue.y &&
          localVoxels[i].z == localMemClearValue.z &&
          localVoxels[i].w == localMemClearValue.w)
      {
...
///
/// @param cornerVoxelKey Key for the lower extents corner of the global work group. All other GPU threads can resolve their key by
                                ...
                                )
{
  ...
  {
    if (effectiveGlobalId.x < (uint)workingVoxelExtents.x &&
        effectiveGlobalId.y < (uint)workingVoxelExtents.y &&
        effectiveGlobalId.z < (uint)workingVoxelExtents.z)
    {
      // Find the region for voxelKey
      if (regionsResolveRegion(&voxelKey, &voxelRegion, &regionVoxelOffset,
                              regionKeysGlobal, regionMemOffsetsGlobal, regionCount, sizeof(*voxelOccupancy)))
      {
        // Found the region memory.
        // Index into voxels
        const uint vidx = regionVoxelOffset +
                          voxelKey.voxel[0] +
                                 ...
                                 int batch)
{
  ...
  uint regionVoxelOffset;
  regionsInitCurrent(&voxelRegion, &regionVoxelOffset);
  calculateFacePadding(workingVoxelExtents, outerRegionPadding,
  ...
  for (int i = 0; i < batch; ++i)
  {
    ...
    if (workingIndex3.x < -outerRegionPadding.x || workingIndex3.y < -outerRegionPadding.y || workingIndex3.z < -outerRegionPadding.z ||
        workingIndex3.x >= workingVoxelExtents.x + outerRegionPadding.x || workingIndex3.y >= workingVoxelExtents.y + outerRegionPadding.y ||
        workingIndex3.z >= workingVoxelExtents.z + outerRegionPadding.z ||
        (0 <= workingIndex3.x && workingIndex3.x < workingVoxelExtents.x &&
         0 <= workingIndex3.y && workingIndex3.y < workingVoxelExtents.y &&
         0 <= workingIndex3.z && workingIndex3.z < workingVoxelExtents.z))
    {
      printf("Bad working index: %u => %d %d %d: region %d %d %d, padding %d %d %d\n",
             voxelIndex, workingIndex3.x, workingIndex3.y, workingIndex3.z,
        ...
        for (int z = -1; z <= 1; ++z)
        {
          for (int y = -1; y <= 1; ++y)
          {
            for (int x = -1; x <= 1; ++x)
            {
              ...
              {
                // Index into workingVoxels.
                const uint ridx = regionVoxel3.x +
                                  regionVoxel3.y * workingVoxelExtents.x +
                ...
                char4 obstructionValue = make_char4(
                  workingIndex3.x - regionVoxel3.x,
                  workingIndex3.y - regionVoxel3.y,
...
/// @par Invocation
/// In layer batches, one thread per X/Z coordinate, processing zbatch items in Z.
__kernel void propagateObstacles(__global char4 *srcVoxels,
                                 __global char4 *dstVoxels,
                                 int3 voxelExtents,
                                 float searchRange,
                                 float3 axisScaling
                               , __local char4 *localVoxels
                                )
{
  ...
  const uint voxelIndex = get_global_id(0) + get_global_id(1) * voxelExtents.x + get_global_id(2) * voxelExtents.x * voxelExtents.y;
  const bool validVoxel = voxelIndex < volumeOf(voxelExtents) &&
    effectiveGlobalId.x < voxelExtents.x && effectiveGlobalId.y < voxelExtents.y && effectiveGlobalId.z < voxelExtents.z;

  // if (isGlobalThread(0, 0, 0))
  // {
  //   printf("axisScaling: %f %f %f\n", axisScaling.x, axisScaling.y, axisScaling.z);
  // }
  loadPropagationLocalVoxels(srcVoxels, localVoxels, voxelExtents,
                             effectiveGlobalId, effectiveLocalId, effectiveLocalSize);
  ...
  const float3 currentScaledSeparation = make_float3(closestObstruction.x, closestObstruction.y, closestObstruction.z)
                                        * axisScaling;
...
///   The w coordinate is zero if there is no obstructed in range and the voxel itself isn't and obstructed.
///   The w coordinate is 1 if there is an obstructed to consider.
/// @param regionKeysGlobal Array of region keys matched to elements in @p regionMemOffsetsGlobal (see @ref voxelClearanceGpu).
/// @param regionMemOffsetsGlobal Array of memory offsets into @p voxelsMem where to find each region (see @ref voxelClearanceGpu).
...
__kernel void migrateResults(__global float *clearanceVoxels,
                             __global char4 *workingVoxels,
                             int3 regionVoxelDimensions,
                             int3 workingVoxelExtents,
                             float searchRange,
                             ...
                             uint flags)
{
  ...
  if (0 <= regionIndex3.x && regionIndex3.x < regionVoxelDimensions.x &&
      0 <= regionIndex3.y && regionIndex3.y < regionVoxelDimensions.y &&
      0 <= regionIndex3.z && regionIndex3.z < regionVoxelDimensions.z)
  {
    ...
    const uint widx = workingIndex3.x +
                      workingIndex3.y * workingVoxelExtents.x +
                      workingIndex3.z * workingVoxelExtents.x * workingVoxelExtents.y;
    const char4 nearestObstacleOffset = workingVoxels[widx];
    const float3 nearestObstacle = make_float3(
                                        nearestObstacleOffset.x * voxelResolution * axisScaling.x,
    ...
    const uint vidx = regionIndex3.x +
                      regionIndex3.y * regionVoxelDimensions.x +
