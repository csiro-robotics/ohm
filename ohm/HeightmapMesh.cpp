...
bool HeightmapMesh::buildMesh(const Heightmap &heightmap)
{
  ...
  if (!delaunay.triangles.empty())
  {
    ...
    for (size_t i = 0; i < delaunay.triangles.size(); i += 3)
    {
      ...
      imp_->edges.push_back(TriangleEdge(indices[0], indices[1], tri_count, 0));
      imp_->edges.push_back(TriangleEdge(indices[1], indices[2], tri_count, 1));
      imp_->edges.push_back(TriangleEdge(indices[2], indices[0], tri_count, 2));
