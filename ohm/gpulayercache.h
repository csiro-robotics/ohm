...
#ifndef GPULAYERCACHE_H_
#define GPULAYERCACHE_H_
...
namespace ohm
{
  ...
  ///
  /// The cache may be used to ensure voxels from a @c MapChunk::voxelMaps associated with a @c MapLayer are uploaded
  ...
  public:
    ...
    /// @param gpuQueue GPU queue used to execute memory transfer in.
    /// @param map The map to which the cache belongs.
    /// @param layerIndex Identifies @c MapLayer from which to cache voxel data.
    /// @param targetGpuMemSize The maximum allowed buffer size (bytes).
    /// @param flags Creation flags: see @c GpuCacheFlag. @c GCFRead is currently mandatory.
    GpuLayerCache(const gputil::Device &gpu, const gputil::Queue &gpuQueue, OccupancyMap &map, unsigned layerIndex,
                  size_t targetGpuMemSize, unsigned flags);
    ...
    /// Identifies the voxel layer the GPU cache operates on. See @c MapLayout and @c MapChunk::voxelMaps.
    ...
    /// @param regionKey The key for the region to upload.
    ...
    /// @param batchMarker Optionally set to demarcate a group of upload operations.
    ...
    size_t allocate(OccupancyMap &map, const glm::i16vec3 &regionKey, MapChunk *&chunk, gputil::Event *event,
                    CacheStatus *status = nullptr, unsigned batchMarker = 0, unsigned flags = 0u);
    ...
    /// @return The byte offset into @c buffer() where the region is being uploaded. ~0u on
    ///     failure to upload as determined by the use of the @p batchMarker.
    size_t upload(OccupancyMap &map, const glm::i16vec3 &regionKey, MapChunk *&chunk, gputil::Event *event,
                  CacheStatus *status = nullptr, unsigned batchMarker = 0, unsigned flags = 0u);
    ...
    /// @param regionKey The key for the map chunk of interest.
    /// @param[out] offset Set to the byte offset in @c buffer() if in the cache. May be null.
    /// @param[out] currentEvent Set to the most recent GPU data manipulation event associated with this chunk.
    ///   May be null.
    /// @return True if the region is cached, false otherwise.
    bool lookup(OccupancyMap &map, const glm::i16vec3 &regionKey, size_t *offset, gputil::Event *currentEvent = nullptr);
    ...
    /// @param batchMarker The maker to match against.
    /// @param event The most recent event to associate.
    void updateEvents(unsigned batchMarker, gputil::Event &event);
    ...
    /// @overload
    void syncToMainMemory(const glm::i16vec3 &regionKey);
  ...
  private:
    ...
    GpuCacheEntry *resolveCacheEntry(OccupancyMap &map, const glm::i16vec3 &regionKey, MapChunk *&chunk,
                                     gputil::Event *event, CacheStatus *status, unsigned batchMarker,
                                     unsigned flags, bool upload);

    void allocateBuffers(const OccupancyMap &map, const MapLayer &layer, size_t targetGpuMemSize);

    void syncToMainMemory(GpuCacheEntry &entry, bool waitOnSync);

    GpuCacheEntry *findCacheEntry(const glm::i16vec3 &regionKey);
    const GpuCacheEntry *findCacheEntry(const glm::i16vec3 &regionKey) const;

    GpuCacheEntry *findCacheEntry(const MapChunk &chunk);
    const GpuCacheEntry *findCacheEntry(const MapChunk &chunk) const;

    GpuLayerCacheDetail *_imp;
  };
