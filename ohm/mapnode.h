...
#endif // !__OPENCL_C_VERSION__
  inline coord_real regionCentreCoord(int regionCoord, const coord_real regionDimesion)
  {
    return regionCoord * regionDimesion;
  }

#ifndef __OPENCL_C_VERSION__
  template <typename coord_real>
#endif // !__OPENCL_C_VERSION__
  inline int pointToRegionVoxel(coord_real coord, coord_real voxelResolution, coord_real regionResolution)
  {
    ...
    else if (coord >= regionResolution && coord - epsilon < regionResolution)
    {
      coord -= epsilon;
    }
    return (int)floor((coord / voxelResolution));
  }
