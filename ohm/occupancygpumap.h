...
#ifndef OCCUPANCYGPUMAP_H_
#define OCCUPANCYGPUMAP_H_
...
namespace ohm
{
  ...
  namespace gpumap
  {
    ...
    ///
    /// @param map The map to enable GPU usage on.
    /// @param layerGpuMemSize GPU memory buffer size per map layer.
    /// @param mappableBuffers True to use mapped GPU buffers, false to use queued data transfer.
    /// @return The @c GpuCache for the map. Null if GPU code is not enabled.
    GpuCache *ohm_API enableGpu(OccupancyMap &map, size_t layerGpuMemSize, bool mappableBuffers = true);
    ...
    /// Does nothing if the map has no GPU cache or GPU code is disabled.
    /// @param map The map to sync GPU memory for.
    /// @param layerIndex The index of the layer to sync.
    void ohm_API sync(OccupancyMap &map, unsigned layerIndex);
  ...
  public:
    ...
    static const double DefaultMaxRange;
    ...
    /// @param borrowedMap True to borrow the map, @c false for this object to take ownership.
    /// @param expectedPointCount The expected point count for calls to @c integrateRays(). Used as a hint.
    /// @param gpuMemSize Optionally specify the target GPU cache memory to allocate.
    GpuMap(OccupancyMap *map, bool borrowedMap = true, unsigned expectedPointCount = 2048, size_t gpuMemSize = 0u);
    ...
    unsigned integrateRays(const glm::dvec3 *rays, unsigned pointCount, bool endPointsAsOccupied = true);
  ...
  private:
    ...
    /// @param bufferIndex Identifies the batch to wait on.
    void waitOnPreviousOperation(int bufferIndex);
    ...
    ///
    /// @param regionHash The hash code for the region of interest.
    /// @param regionKey The key for the region of interest.
    /// @param[in,out] regionsBuffer GPU buffer to upload @c regionKey to. Will be assigned a different buffer when the
    ///   @c GpuLayerCache is full.
    /// @param[in,out] offsetsBuffer GPU buffer to upload the memory offset for the region to. Will be assigned a
    ///   different buffer when the @c GpuLayerCache is full.
    /// @param endPointsAsOccupied When @c true, the end points of the rays increase the occupancy probability.
    ///   Otherwise they decrease the probability just as the rest of the ray.
    /// @param allowRetry Allow recursion when the @c GpuLayerCache?
    void enqueueRegion(unsigned regionHash, const glm::i16vec3 &regionKey,
                       gputil::PinnedBuffer &regionsBuffer, gputil::PinnedBuffer &offsetsBuffer,
                       bool endPointsAsOccupied, bool allowRetry);

    /// Finalise the current ray/region batch and start executing GPU kernel.
    /// @param[in,out] regionsBuffer GPU buffer containing uploaded region keys. Will be unpinned.
    /// @param[in,out] offsetsBuffer GPU buffer containing memory offsets for regions. Will be unpinned.
    /// @param endPointsAsOccupied When @c true, the end points of the rays increase the occupancy probability.
    ///   Otherwise they decrease the probability just as the rest of the ray.
    void finaliseBatch(gputil::PinnedBuffer &regionsBuffer, gputil::PinnedBuffer &offsetsBuffer, bool endPointsAsOccupied);

    GpuMapDetail *_imp;
  };
}

#endif // OCCUPANCYGPUMAP_H_
