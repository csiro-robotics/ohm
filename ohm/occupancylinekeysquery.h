...
#ifndef OCCUPANCYLINEKEYSQUERY_H_
#define OCCUPANCYLINEKEYSQUERY_H_
...
namespace ohm
{
  ...
  public:
    ...
    /// @param queryFlags Flags controlling the query behaviour. See @c QueryFlag and @c LineKeysQuery::Flag.
    LineKeysQuery(ohm::OccupancyMap &map, unsigned queryFlags = 0u);

    /// Construct a new query using the given parameters.
    /// @param queryFlags Flags controlling the query behaviour. See @c QueryFlag and @c LineKeysQuery::Flag.
    LineKeysQuery(unsigned queryFlags = 0);
    ...
    /// @param pointCount Number of elements in @p rays. Expected to be an even number; i.e., twice the number of rays.
    void setRays(const glm::dvec3 *rays, size_t pointCount);
  ...
  protected:
    ...
    bool onWaitAsync(unsigned timeoutMs) override;
...
#endif // OCCUPANCYLINEKEYSQUERY_H_
