...
#ifndef OHMCLEARANCEPROCESS_H_
#define OHMCLEARANCEPROCESS_H_
...
namespace ohm
{
  ...
  public:
    ...
    enum QueryFlag
    {
      ...
      QF_InstantiateUnknown = (QF_Specialised << 0),
    };
    ...
    /// @param queryFlags Flags controlling the query behaviour. See @c QueryFlag and @c ClearanceProcess::QueryFlag.
    ClearanceProcess(float searchRadius, unsigned queryFlags);
    /// Destructor.
    ~ClearanceProcess();
    ...
    /// @param timeSlice The amount of time available for processing (seconds). Stop if exceeded.
    /// @return See @c MappingProcessResult.
    int update(OccupancyMap &map, double timeSlice) override;
    ...
    ///
    /// @param map The map to process.
    /// @param minExtents The minimum extents corner of the region to calculate.
    /// @param maxExtents The maximum extents corner of the region to calculate.
    ...
    void calculateForExtents(OccupancyMap &map, const glm::dvec3 &minExtents, const glm::dvec3 &maxExtents, bool force = true);

  protected:

    /// Update clearance for the given region.
    /// @param map The operating map.
    /// @param regionKey The key of the region to update.
    /// @param force Force update => update even if not dirty.
    /// @return True if work was done. False if nothing need be done.
    bool updateRegion(OccupancyMap &map, const glm::i16vec3 &regionKey, bool force);

    ClearanceProcessDetail *imp();
    const ClearanceProcessDetail *imp() const;

  private:
    ClearanceProcessDetail *_imp;
  };
}

#endif // OHMCLEARANCEPROCESS_H_
