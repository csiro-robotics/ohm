...
#ifndef OHMMAPLAYOUT_H_
#define OHMMAPLAYOUT_H_
...
namespace ohm
{
  ...
  /// @c MapChunk::voxelMaps which store voxel data in a series of independent layers. The @c MapLayout defines the
  /// number of layers and identifies the data structure of each layer (via @c MapLayer).
  ///
  /// Each @c MapLayer is named, indexes a specific array in @c MapChunk::voxelMaps and defines the data contained
  ...
  public:
    ...
    const MapLayer *layer(const char *layerName) const;
  ...
  private:
    MapLayoutDetail *_detail;
  };
}

#endif // OHMMAPLAYOUT_H_
