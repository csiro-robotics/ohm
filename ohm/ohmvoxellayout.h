...
#ifndef OHMVOXELLAYOUT_H_
#define OHMVOXELLAYOUT_H_
...
namespace ohm
{
  ...
  public:
    ...
    /// @param detail The @c VoxelLayoutDetail.
    template <typename U>
    inline VoxelLayoutT(U *detail = nullptr) : _detail(detail) {}
    ...
    DataType::Type memberType(size_t memberIndex) const;

    /// Query the byte size to the member at @p memberIndex.
    /// @param memberIndex The index of the member to query.
    /// @return The byte size to @p memberIndex or zero if out of range.
    size_t memberSize(size_t memberIndex) const;

    /// Query the desired clear value for the member at member at @p memberIndex.
    /// @param memberIndex The index of the member to query. Must be in range [0, @c memberCount()).
    /// @return The clear value for @p memberIndex.
    uint64_t memberClearValue(size_t memberIndex) const;
    ...
    const void *memberPtr(size_t memberIndex, const void *mem) const;
    /// @overload
    void *memberPtr(size_t memberIndex, void *mem) const;
    ...
    template <typename D>
    inline const D *memberData(size_t memberIndex, const void *mem) const
    {
      if (sizeof(D) == memberSize(memberIndex))
      {
        return reinterpret_cast<const D *>(memberPtr(memberIndex, mem));
      }
      return nullptr;
    }
    ...
    /// @overload
    template <typename D>
    inline D *memberData(size_t memberIndex, void *mem) const
    {
      if (sizeof(D) == memberSize(memberIndex))
      {
        return reinterpret_cast<D *>(memberPtr(memberIndex, mem));
      }
      return nullptr;
    }
    ...
    inline T *detail() const { return _detail; }
  ...
  public:
    ...
    /// @param clearValue The value used with memset to initialise the data member.
    void addMember(const char *name, DataType::Type type, uint64_t clearValue);

    /// Assignment operator.
    /// @param other Object to shallow copy.
    inline VoxelLayout &operator=(const VoxelLayout &other) { _detail = other.detail(); return *this; }
  };
  ...
  public:
    ...
    /// Assignment operator.
    /// @param other Object to shallow copy.
    inline VoxelLayoutConst &operator=(const VoxelLayoutConst &other) { _detail = other.detail(); return *this; }

    /// Assignment operator from writable object.
    /// @param other Object to shallow copy.
    inline VoxelLayoutConst &operator=(const VoxelLayout &other) { _detail = other.detail(); return *this; }
  };
  ...
  template <typename T>
  VoxelLayoutT<T>::VoxelLayoutT(VoxelLayoutT &&other)
    : _detail(other._detail)
  {
