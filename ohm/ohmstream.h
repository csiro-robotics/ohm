...
namespace ohm
{
  ...
  public:
    ...
    bool open(const char *filePath, unsigned flags = 0u);
  ...
  protected:
    ...
    /// @param flags @c StreamFlag values to open with.
    /// @return True on success.
    virtual bool doOpen(const char *filePath, unsigned flags = 0u) = 0;
    ...
    /// Constructor.
    /// @param imp The implementation pointer, set to @c _imp. Derivations must clean it up.
  ...
  public:
    ...
    /// checking @c isOpen().
    ///
    /// @param filePath Optionally specifies the file to open.
    /// @param flags The @c StreamFlag set to open with.
    InputStream(const char *filePath = nullptr, unsigned flags = 0u);
    ...
    /// @param maxBytes Maximum number of bytes to read. @p buffer must be large enough for this content.
    /// @return The number of bytes read. Zero indicates no bytes available, or an decompression error.
    unsigned read(void *buffer, unsigned maxBytes);
    ...
    /// @param maxBytes Maximum number of bytes to read. @p buffer must be large enough for this content.
    /// @return The number of bytes read. Zero indicates no bytes available, or an decompression error.
    unsigned readRaw(void *buffer, unsigned maxBytes);
  ...
  protected:
    ...
    /// @param flags @c StreamFlag values to open with.
    /// @return True on success.
    bool doOpen(const char *filePath, unsigned flags = 0u) override;
  ...
  public:
    ...
    /// @param filePath Optionally specifies the file to open.
    /// @param flags The @c StreamFlag set to open with.
    OutputStream(const char *filePath = nullptr, unsigned flags = 0u);

    /// Destructor. Flushes and closes the file.
    ~OutputStream();

    /// Write bytes to the file, compression enabled.
    unsigned write(const void *buffer, unsigned maxBytes);

    /// Write bytes to the file, with no compression.
    ///
    /// This may only be used before calls to @c write(), or after a call to
    /// @c flush().
    unsigned writeUncompressed(const void *buffer, unsigned maxBytes);
