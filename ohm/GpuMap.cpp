                   ...
                   const RegionWalkFunction &func)
  {
    ...
    for (unsigned i = 0; i < 3; ++i)
    {
      if (direction[i] != 0)
      {
        ...
        time_limit[i] = std::abs((end_point_local[i] - start_point_local[i]) * direction_axis_inv);// +0.5f * regionResolution[i];
      }
      else
      {
  ...
  : imp_(new GpuMapDetail(map, borrowed_map))
{
  ...
  for (unsigned i = 0; i < GpuMapDetail::kBuffersCount; ++i)
  {
    ...
    imp_->region_offset_buffers[i] = gputil::Buffer(gpu_cache.gpu(), sizeof(gputil::ulong1) * prealloc_region_count, gputil::kBfReadHost);
  }
...
unsigned GpuMap::integrateRays(const glm::dvec3 *rays, unsigned point_count, bool end_points_as_occupied)
{
  ...
  gputil::PinnedBuffer ray_buffer(imp_->ray_buffers[buf_idx], gputil::kPinWrite);
  ...
  ray_buffer.unpin(&layer_cache.gpuQueue(), nullptr, &imp_->ray_upload_events[buf_idx]);
  ...
  for (auto &&regionimp_s : imp_->regions)
  {
    enqueueRegion(regionimp_s.first, regionimp_s.second, regions_buffer, offsets_buffer, end_points_as_occupied, true);
  }
...
void GpuMap::enqueueRegion(unsigned region_hash, const glm::i16vec3 &region_key,
                           ...
                           bool end_points_as_occupied, bool allow_retry)
{
  ...
  GpuLayerCache &layer_cache = *gpu_cache.layerCache(kGcIdOccupancy);
  mem_offset = gputil::ulong1(layer_cache.upload(*imp_->map, region_key, chunk, &upload_event, &status, imp_->batch_marker,
  ...
  if (status != GpuLayerCache::kCacheFull)
  {
    ...
    regions_buffer.write(&gpu_region_key, sizeof(gpu_region_key), imp_->region_counts[buf_idx] * sizeof(gpu_region_key));
  ...
  else if (allow_retry)
  {
    ...
    gputil::copyBuffer(imp_->ray_buffers[buf_idx], imp_->ray_buffers[previous_buf_idx],
                       &gpu_cache.gpuQueue(), nullptr, &imp_->ray_upload_events[previous_buf_idx]);
    ...
    {
      // Size the region buffers.
      imp_->region_key_buffers[buf_idx].gputil::Buffer::elementsResize<gputil::int3>(imp_->regions.size() - regions_processed);
      imp_->region_offset_buffers[buf_idx].gputil::Buffer::elementsResize<gputil::ulong1>(imp_->regions.size() - regions_processed);
...
void GpuMap::finaliseBatch(gputil::PinnedBuffer &regions_buffer, gputil::PinnedBuffer &offsets_buffer, bool end_points_as_occupied)
{
  const int buf_idx = imp_->next_buffers_index;
  ...
  updateRegion(layer_cache.gpuQueue(),
               *layer_cache.buffer(),
               ...
               imp_->region_counts[buf_idx],
               imp_->ray_buffers[buf_idx], imp_->ray_counts[buf_idx],
               map->region_voxel_dimensions, map->resolution,
               map->miss_value, (end_points_as_occupied) ? map->hit_value : map->miss_value,
               map->min_node_value, map->max_node_value,
               { imp_->ray_upload_events[buf_idx], imp_->region_key_upload_events[buf_idx], imp_->region_offset_upload_events[buf_idx] },
  ...
  // std::cout << imp_->region_counts[bufIdx] << " regions\n" << std::flush;
