...
namespace ohm
{
  ...
  public:
    ...
    ///
    /// @param timeSliceSec The allowed time slice for this update (seconds). Zero for full update.
    /// @return The overall status. See @c MappingProcessResult.
    int update(double timeSliceSec);
    ...
    inline MapperDetail *detail() { return _detail; }
    /// @overload
    inline const MapperDetail *detail() const { return _detail; }

  private:
    MapperDetail *_detail;
  };
}
