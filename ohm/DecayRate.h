...
#ifndef OHM_DECAYRATE_H
#define OHM_DECAYRATE_H
...
/// Query the decay rate for the voxel at @p key . The decay rate is calculated as the number of hits (samples) in a
...
///
/// @todo Should the traversal value be scaled by the voxel resolution? That would make the @c decayRate() value
/// independent of voxel size.
///
/// @param map The map object to query.
/// @param key The key identifying the voxel of interest.
/// @return The decay rate or zero on failure.
float ohm_API decayRate(const OccupancyMap &map, const Key &key);

/// Calculate the decay rate given a traversal layer voxel and the corresponding @c VoxelMean . This function assumes
/// that both are referencing the same voxel key.
///
/// The decay rate is calculated as the @c `VoxelMean::count / traversal`. The result is zero if the mean count is zero
...
/// @return The decay rate or zero on failure.
inline float ohm_API decayRate(const Voxel<const float> &traversal_voxel, const Voxel<const VoxelMean> &mean_voxel)
{
